...
const AlertIcon = () => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path fillRule="evenodd" d="M9.401 3.003c1.155-2 4.043-2 5.197 0l7.355 12.748c1.154 2-.29 4.5-2.599 4.5H4.645c-2.309 0-3.752-2.5-2.598-4.5L9.4 3.003zM12 8.25a.75.75 0 01.75.75v3.75a.75.75 0 01-1.5 0V9a.75.75 0 01.75-.75zm0 8.25a.75.75 0 100-1.5.75.75 0 000 1.5z" clipRule="evenodd" /></svg>;
const SearchIcon = () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" /></svg>;
...
export default function Profile() {
  ...
  // UI States
  const [activeTab, setActiveTab] = useState("pets"); // 'pets' or 'posts'
  const [loading, setLoading] = useState(true);
  const [postsLoading, setPostsLoading] = useState(false);
  const [actionLoading, setActionLoading] = useState(null); // ID of pet being processed
  ...
  }, [authLoading, user, router, fetchUserPets, activeTab]);
  ...
  return (
    <div className="min-h-screen bg-[#E2F4EF] relative pb-20">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
              <div className="bg-white rounded-[2.5rem] max-w-2xl w-full p-8 shadow-2xl border-4 border-white overflow-hidden flex flex-col max-h-[85vh]">
                  <div className="flex justify-between items-center mb-6">
                      ...
                      <button onClick={() => setShowAvatarModal(false)} className="text-gray-400 hover:text-gray-600 text-2xl">×</button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto grid grid-cols-3 sm:grid-cols-4 gap-4 p-2">
                      {AVATAR_OPTIONS.map((url, idx) => (
                          ...
                          >
                              ...
                              {avatarSaving && <div className="absolute inset-0 bg-white/60 flex items-center justify-center"><div className="w-6 h-6 border-2 border-[#4A90E2] border-t-transparent rounded-full animate-spin"></div></div>}
                          </button>
                      ))}
                  </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-24 md:pt-28 relative z-10">
        
        {/* CHANGE: Use items-start to allow sticky behavior on the child */}
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          
          {/* --- LEFT: USER CARD (STICKY) --- */}
          {/* CHANGE: Added 'sticky top-28 h-fit' directly here */}
          <div className="lg:w-1/3 lg:sticky lg:top-28 h-fit z-20">
                <div className="glass-panel p-8 rounded-[2.5rem] text-center shadow-2xl border border-white/60 backdrop-blur-xl bg-white/80">
                
                <div className="relative w-32 h-32 mx-auto mb-6 group cursor-pointer" onClick={() => setShowAvatarModal(true)}>
          ...
          <div className="lg:w-2/3 w-full min-h-screen">
            ...
            <div className="flex justify-between items-center mb-6 pl-2">
                <div className="flex bg-white p-1.5 rounded-full shadow-sm border border-gray-200">
                    <button 
                        ...
                        className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${activeTab === 'pets' ? 'bg-[#E2F4EF] text-[#4A90E2] shadow-inner' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Our Pets ({pets.length})
                    </button>
                    <button 
                        onClick={() => setActiveTab('posts')}
                        className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${activeTab === 'posts' ? 'bg-[#E2F4EF] text-[#4A90E2] shadow-inner' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Community Posts
                    </button>
                </div>
            ...
            {activeTab === 'pets' && (
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    if (isLegacyAdopted) {
                        effectivePet = { ...pet, adoptionLog: { adoptionDate: new Date(), newOwnerName: completedReq.requesterName, previousOwnerName: "Previous Owner", certificateId: `LEGACY-${pet._id}` } };
                    }
                    ...
                    return (
                        <div 
                            ...
                            className={`bg-white rounded-[2rem] p-6 shadow-sm border transition-all duration-300 relative overflow-hidden ${
                                isIncoming ? "border-purple-200 bg-purple-50/50" : pet.isLost ? "border-red-200 bg-red-50" : "border-white"
                            }`}
                        >
                        {pet.isLost && (
                        ...
                        <div className="flex flex-col sm:flex-row gap-5 relative z-10 mt-2">
                            <div className="relative shrink-0">
                                <div className={`w-24 h-24 rounded-2xl overflow-hidden shadow-sm border-2 relative ${pet.isLost ? 'border-red-400' : 'border-gray-100'}`}>
                                    <Image src={pet.imageUrls[0] || "/imgs/dog.jpg"} alt={pet.name} fill className={`object-cover ${pet.isLost ? 'grayscale' : ''}`} />
                                </div>
                            ...
                            <div className="flex-1 pt-1">
                                <div className="flex flex-wrap items-start justify-between gap-2">
                                    ...
                                    <div className="flex flex-col items-end gap-1">
                                        ...
                                        {pet.isPregnant && <span className="bg-pink-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">PREGNANT</span>}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* --- ACTION BUTTONS ROW --- */}
                        <div className="flex flex-wrap gap-3 mt-5 pt-5 border-t border-gray-100 relative z-10">
                            ...
                            {!isIncoming && (
                                <button 
                                    ...
                                    className={`flex-1 py-2.5 rounded-xl font-bold text-white transition-colors text-sm shadow-sm flex items-center justify-center gap-2 ${
                                        pet.isLost ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'
                                    }`}
                                >
                        ...
                        {/* --- PREGNANCY / MATING BANNERS --- */}
                        <div className="space-y-2 mt-3">
                            {pet.isPregnant && (
                                <Link href={`/pregnancy-tracker/${pet._id}`} className="block w-full py-3 bg-pink-50 border border-pink-100 text-pink-600 rounded-xl font-bold hover:bg-pink-100 transition-colors text-center text-sm flex items-center justify-center gap-2">
                        ...
                        {showCertificate && (
                            <div className="mt-4 p-4 bg-emerald-50 border border-emerald-100 rounded-xl text-center">
                                <h4 className="text-emerald-800 font-bold text-sm mb-1">🎉 Adoption Complete!</h4>
                                <div className="scale-90 origin-center"><DownloadCertificate pet={effectivePet} /></div>
                            </div>
                        )}
                        ...
                        <div className="mt-4 space-y-2">
                            ...
                            {!isIncoming && pet.matingHistory?.map((req, idx) => (
                                ['accepted', 'ownerConfirmedMating', 'requesterConfirmedMating', 'mated'].includes(req.status) && <div key={idx} className="scale-95"><MatingConfirmation pet={pet} request={req} onUpdate={fetchUserPets} /></div>
                            ))}
                            {!isIncoming && pet.outgoingRequests?.map((req, idx) => (
                                req.requestType === "mating" && <div key={idx} className="scale-95"><MatingConfirmation pet={pet} request={req} onUpdate={fetchUserPets} /></div>
                            ))}
                            {!isIncoming && !isLegacyAdopted && pet.adoptionRequests?.map((req, idx) => (
                                (req.status === "approved" || req.status === 'confirmHandover') && !(req.ownerConfirmedHandover && req.requesterConfirmedHandover) && <div key={idx} className="scale-95"><AdoptionHandover pet={pet} request={req} onUpdate={fetchUserPets} isIncoming={false} /></div>
                            ))}
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-white/60 rounded-[2.5rem] border border-white shadow-sm text-center">
                    ...
                    <button onClick={() => router.push("/Addpet")} className="mt-6 bg-[#4A90E2] text-white px-6 py-2 rounded-full font-bold hover:shadow-lg transition">Add Your First Pet</button>
                </div>
                )}
                </>
            )}
            ...
            {activeTab === 'posts' && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    {postsLoading ? (
                        ...
                        userPosts.map(post => (
                            <div key={post._id} className="bg-white/90 backdrop-blur-md rounded-[2rem] p-6 shadow-sm border border-white relative group hover:shadow-md transition-shadow">
                                <div className="flex justify-between items-start mb-3">
                                    <h3 className="text-lg font-bold text-gray-800 leading-tight pr-8">{post.title}</h3>
                                    <button onClick={() => handleDeletePost(post._id)} className="text-gray-300 hover:text-red-500 transition-colors p-2 hover:bg-red-50 rounded-full">
                                        <TrashIcon />
                                    </button>
                                </div>
                                ...
                                <div className="flex justify-between items-center text-xs text-gray-400 font-bold uppercase tracking-wider pt-2 border-t border-gray-100">
                                    ...
                                    <Link href={`/community/${post._id}`} className="text-[#4A90E2] hover:underline flex items-center gap-1">View Discussion <span className="text-lg">&rarr;</span></Link>
                                </div>
                            </div>
                        ))
                    ) : (
                        <div className="text-center py-20 bg-white/60 rounded-[2.5rem] border border-white shadow-sm">
                            ...
                            <Link href="/community" className="mt-4 inline-block text-[#4A90E2] font-bold hover:underline">Go to Community to Post</Link>
                        </div>
                    )}
                </div>
            )}
