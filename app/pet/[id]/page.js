...
  const [requesterPets, setRequesterPets] = useState([]); // NEW STATE
  const [requesterPetId, setRequesterPetId] = useState(""); // NEW STATE
...
      setPet(data);
      
      // Fetch requester's pets if logged in
      if (user) {
        await fetchRequesterPets(user.uid, data.type);
      }
    } catch (err) {
      console.error(err);
    }
  };
  
  // NEW FUNCTION: Fetch requester's pets of the same type
  const fetchRequesterPets = async (uid, petType) => {
    try {
        // NOTE: This API must be available and return the full pet details
        // The API route droomiweb/pet-mat/pet-mat-ada8729c454ea9490710bdffbc5c19939f7bb8ce/app/api/pet/user/[uid]/route.js needs to return all pet details
        const petsRes = await fetch(`/api/pet/user/${uid}`); 
        if (petsRes.ok) {
            const allPets = await petsRes.json();
            // Filter to only include pets of the same type as the target pet
            const compatiblePets = allPets.filter(p => p.type === petType);
...
            } else {
                setRequesterPetId(""); // Ensure reset if multiple/zero
            }
        }
    } catch (err) {
...
          requesterPetId: selectedRequesterPetId, // NEW FIELD
          requesterPetName: selectedPet?.name, // NEW FIELD
...
  useEffect(() => {
    fetchPet();
  }, [params.id, user?.uid]); // Add user.uid to dependency array
...
  const genderColor = pet.gender === 'Male' ? 'bg-blue-200 text-blue-800' : 'bg-pink-200 text-pink-800';
  ...
  // Determine button disabled status
...
        )}
        
        {/* Name and Basic Info (remains the same) */}
...
        )}
        
        {/* Mating Request Section (UPDATED) */}
...
                  </div>
              )}
              {/* Single pet confirmation or zero pet message */}
              {requesterPets.length === 1 && (
                 <p className="text-sm text-gray-600 mb-4">Request will be sent for your pet: **{requesterPets[0].name}**.</p>
              )}
              {requesterPets.length === 0 && user && (
                 <p className="text-red-500 font-semibold mb-4">You have no registered pets of type {pet.type} to request a mating.</p>
              )}
...
                    pet.verificationStatus === 'verified' && requesterPets.length > 0 && !!requesterPetId // FIX: Added check for requesterPetId
                    ? "bg-[#4A90E2] hover:bg-[#3A75B9] text-white"
                    : "bg-gray-400 text-gray-700 cursor-not-allowed"
                }`}
                disabled={pet.verificationStatus !== 'verified' || requesterPets.length === 0 || !requesterPetId}
              >
...
        {/* Message History Section (remains the same) */}
        <div className="mt-8 pt-6 border-t border-gray-200">
...
        </div>
        
        {/* Mating History Section (remains the same) */}
        <div className="mt-8 pt-6 border-t border-gray-200">
